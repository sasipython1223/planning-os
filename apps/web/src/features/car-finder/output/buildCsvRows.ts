...
export function buildCsvRows(ranked: ScoredListing[]): string[][] {
  const rows = ranked.map((item, index) => [
    ...
    String(item.monthlyAssessment.monthlyInstallment ?? ""),
    item.monthlyAssessment.source + (item.monthlyAssessment.indicativeOnly ? ":indicative" : ""),
    String(item.listing.coeRemainingMonths ?? ""),
    String(item.listing.deregValue ?? ""),
    String(item.listing.estimatedParf ?? ""),
    item.paperValueAssessment.source + (item.paperValueAssessment.indicativeOnly ? ":indicative" : ""),
